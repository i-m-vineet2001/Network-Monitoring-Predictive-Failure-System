...
# def get_logs_by_range(range_type="all"):
#     """
#     range_type:
#     all
#     day
...
#     if range_type == "day":
#         querry["timestamp"] = {"$gte": now - timedelta(days=1)}
#     elif range_type == "week":
#         querry["timestamp"] = {"$gte": now - timedelta(weeks=1)}
#     elif range_type == "month":
#         querry["timestamp"] = {"$gte": now - timedelta(days=30)}

#     logs = list(logs_collection.find(querry).sort("timestamp", -1).limit(200))
...
#     if logs_collection is None:
#         # MongoDB not configured/available; skip insert silently
#         return
...
#         print(f"[ERROR] Failed to insert log for {node_name}: {str(e)[:100]}")
